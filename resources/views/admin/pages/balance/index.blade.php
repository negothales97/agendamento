            ...
            <div class="row">
                ...
                <div class="col-lg-4 col-xs-4">
                    ...
                    <div class="small-box bg-gray">
                        ...
                        <a href="{{ route('admin.balance.transfer') }}" class="small-box-footer btn-balance-transfer"
                                ...
                                <tbody>
                                    @forelse ($transactions as $transaction)
                                        <tr>
                                            ...
                                            <td>{{ $transaction->specialist->name ?? '-' }}</td>
                                            <td>{{ $transaction->specialist->email ?? '-' }}</td>
                                            <td>
                                                {{ $transaction->type == 'transfer' || $transaction->type == 'chargeback' ? '-' : '' }}
                                                R$ {{ formatIntegerToDecimal($transaction->value) }}
                                            </td>
                                            <td>
                                                @if($transaction->value_avivare == 0)
                                                 -
                                                @else
                                                {{ $transaction->type == 'transfer' || $transaction->type == 'chargeback' ? '-' : '' }}
                                                R$ {{formatIntegerToDecimal($transaction->value_avivare)}}
                                                @endif
                                            </td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="8">Nenhuma transação realizada</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
